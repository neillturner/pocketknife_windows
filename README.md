...
pocketknife_windows is a modification of orginal pocketknife for windows 2008 servers and above. 
...
* Install `pocketknife_windows`: `gem install pocketknife_windows`
...
Create a new *project*, a special directory that will contain your configuration files. For example, create the `swa` project directory by running:
    ...
    pocketknife_windows --create swa
...
If something goes wrong while deploying the configuration, you can display verbose logging from `pocketknife` and Chef by using the `-v` option. For example, deploy the configuration to `henrietta` with verbose logging:
    ...
    pocketknife_windows -v henrietta
...
This software is published as open source at https://github.com/neillturner/pocketknife_windows

You can view and file issues for this software at https://github.com/neillturner/pocketknife_windows/issues
...
Modifications by Neill Turner (2012). 
